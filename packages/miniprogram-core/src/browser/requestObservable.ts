...
export function initRequestObservable(
  ...
  function injectTraceHeader<T extends { header?: Record<string, string> }>(
    ...
    // 创建或继承 trace context
    const traceContext = tracing.rootTraceContext
      ? createChildSpan(tracing.rootTraceContext)
      : createTraceContext(true)
  ...
  adapter.request = (options: RequestOptions) =>
    wrapRequest(originalRequest, options);
  wx.request = (options: RequestOptions) =>
    wrapRequest(originalWxRequest, options);
  wx.uploadFile = (options: UploadFileOptions) =>
    wrapUploadFile(originalWxUploadFile, options);
  wx.downloadFile = (options: DownloadFileOptions) =>
    wrapDownloadFile(originalWxDownloadFile, options);
