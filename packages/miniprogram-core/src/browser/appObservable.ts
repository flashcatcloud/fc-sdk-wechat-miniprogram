...
export function initAppObservable(adapter: PlatformAdapter) {
  ...
  const unhandledRejectionObservable =
    new Observable<AppUnhandledRejectionEvent>();

  adapter.onAppShow(() =>
    appObservable.notify({ lifecycle: "show", time: Date.now() })
  );
  adapter.onAppHide(() =>
    appObservable.notify({ lifecycle: "hide", time: Date.now() })
  );
  adapter.onError((message) =>
    errorObservable.notify({ message, time: Date.now() })
  );
