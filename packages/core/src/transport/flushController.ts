...
export function createFlushController(flushInterval: number, batchBytesLimit: number): FlushController {
  const flushObservable = new Observable<FlushEvent>()
  let bytesCount = 0
  let timer = createTimer(() => flushObservable.notify({ reason: 'timer' }), flushInterval)

  function resetTimer() {
    timer.clear()
    timer = createTimer(() => flushObservable.notify({ reason: 'timer' }), flushInterval)
  }
