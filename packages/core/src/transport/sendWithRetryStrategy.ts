...
export function sendWithRetryStrategy(
  ...
  if (
    ...
    send(payload, state, sendStrategy, onRetryExhausted, {
      onSuccess: () => retryQueuedPayloads(RetryReason.AFTER_SUCCESS, state, sendStrategy, onRetryExhausted),
      onFailure: () => {
...
function scheduleRetry(state: RetryState, sendStrategy: SendStrategy, onRetryExhausted: (payload: Payload) => void) {
  ...
  setTimeout(() => {
    ...
    send(payload, state, sendStrategy, onRetryExhausted, {
      onSuccess: () => {
        ...
        retryQueuedPayloads(RetryReason.AFTER_RESUME, state, sendStrategy, onRetryExhausted)
      },
      onFailure: () => {
...
function retryQueuedPayloads(
  ...
  while (previousQueue.size() > 0) {
    sendWithRetryStrategy(previousQueue.dequeue()!, state, sendStrategy, onRetryExhausted)
  }
}
