...
export function persistPayload(adapter: StorageAdapter, payload: Payload) {
  try {
    let persisted: PersistentPayload[] = (adapter.getStorageSync(STORAGE_KEY) as PersistentPayload[]) || []
    ...
    while (persisted.length > MAX_PERSISTED_PAYLOADS || calculateTotalBytes(persisted) > MAX_TOTAL_BYTES) {
      persisted.shift()
    }
...
export function loadAndClearPersistedPayloads(adapter: StorageAdapter): Payload[] {
  try {
    const persisted = (adapter.getStorageSync(STORAGE_KEY) as PersistentPayload[]) || []
    adapter.removeStorageSync(STORAGE_KEY)

    const now = Date.now()
    return persisted.filter((p) => now - p.timestamp < PAYLOAD_EXPIRATION).map(({ timestamp, ...payload }) => payload)
  } catch (e) {
    return []
  }
}
