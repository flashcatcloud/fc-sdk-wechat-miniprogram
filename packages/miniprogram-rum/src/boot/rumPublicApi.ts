...
export function makeRumPublicApi(): RumPublicApi {
  ...
  let strategy: Strategy = createPreStartStrategy(
    adapter,
    (configuration, adapterInstance) => {
      ...
      const nextStrategy: Strategy = {
        ...
        setGlobalContext: (context) =>
          started.globalContext.setContext(context),
  ...
  const rumPublicApi: RumPublicApi = {
    ...
    addError: monitor((message, source, stack) =>
      strategy.addError(message, source || "custom", stack)
    ),
    addTiming: monitor((name, value) => strategy.addTiming(name, value)),
    addCustomEvent: monitor((name, context) =>
      strategy.addCustomEvent(name, context)
    ),
